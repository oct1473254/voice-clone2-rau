...
GERMAN_SCENE = (
    ...
    "GERTRUDE: Sprich nicht weiter, sueßer Sohn.\n"
    "POLONIUS: Wenn dies Wahnsinn ist, so hat er doch Methode.\n"
)
...
def test_script_gen_end_to_end_dry_run(cfg, tmp_path):
    ...
    gen_clients = LLMClients(anthropic_factory=lambda _: StubAnthropic(GENERATED_SCENE))
    english = generate(prompt, "anthropic", cfg.script_gen.models["anthropic"], anthropic_api_key="stub", clients=gen_clients)
    assert english == GENERATED_SCENE

    # 3. Translation — stubbed (separate stub returns German)
    trans_clients = LLMClients(anthropic_factory=lambda _: StubAnthropic(GERMAN_SCENE))
    german = translate(english, cfg, clients=trans_clients)
    assert "Worte" in german

    # 4. Line splitting + filesystem writes for both languages
    parsed_en = split_script(english)
    parsed_de = split_script(german)
    assert parsed_en.characters == ["GERTRUDE", "HAMLET", "POLONIUS"]
    write_split_files(parsed_en, workspace, language="English")
    write_split_files(parsed_de, workspace, language="German")

    # 5. TTS for each English line, DRY_RUN → placeholder files
    voice_map = CharacterVoiceMap(cfg.script_gen.character_voices_path)
    en_output_dir = workspace / "valid_lines" / "English" / "output"
    for line in parsed_en.lines:
        voice_id = voice_map.resolve(line.character)
        out = en_output_dir / f"{line.line_number:03d}-{line.character}.mp3"
    ...
    assert (paths["names"] / "01-GERTRUDE.txt").is_file()
...
def test_script_gen_uses_real_elevenlabs_client_when_not_dry_run(cfg, tmp_path):
    ...
    write_split_files(parsed, workspace, language="English")
